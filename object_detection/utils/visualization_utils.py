...
def draw_bounding_box_on_image(image,
  ...
  # centroid=(cx,cy)
  #print centroid
  # if (chk_exit(ceil(cx),123, prev_cent) and (cx<=125) and (cx>=120)):
	 #      exit_cnt+=1
	 #      # print entry_cnt, exit_cnt
  # if (chk_entry(ceil(cx),123, prev_cent) and (cx<=125) and (cx>=120)):
	 #      entry_cnt+=1
	 #      # print entry_cnt, exit_cnt
...
def visualize_boxes_and_labels_on_image_array(
  ...
  global entry_cnt, exit_cnt, cx, cy
  im=Image.fromarray(np.uint8(image))
  #np.copyto(im, np.array(image))
  im_w, im_h = im.size
  ...
  for box, color in box_to_color_map.items():
    ...
    if keypoints is not None:
      draw_keypoints_on_image_array(
          ...
          use_normalized_coordinates=use_normalizedvisualize_boxes_and_labels_on_image_array_coordinates)
  #print ceil(cx), ceil(cy)
  # ft = cv2.FONT_HERSHEY_SIMPLEX  
  #cv2.putText(image,"Trucks Exited: "+str(exit_cnt)+"  Trucks Entered: "+str(entry_cnt),(235,20), ft, 0.4,(255,0,0),1,cv2.LINE_AA) 
  #cv2.putText(image,'Time: '+time.strftime("%c"),(1167,110), ft, 1,(255,255,255),1,cv2.LINE_AA) 
  # log = 'log.txt'
  # logfile = file(log, "a")
